...
void vertical_order(Node *node, int current, map<int, vector<int>> &list){
	if(node == NULL)
		return
	list[current].push_back(node->data);
	vertical_order(node->left, current-1, list);
	vertical_order(node->right, current+1, list);
}

void print_vertical(Node *node, int current){
	map<int, vector<int>> list;
	vertical_order(node,current,list);
